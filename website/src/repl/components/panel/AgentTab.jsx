...
const DEFAULT_PROMPT_WEBSOCKET_URL = 'wss://tones-inputs-garage-fired.trycloudflare.com/json?channel=1';
...
export function AgentTab({ context }) {
  ...
  useEffect(() => {
    ...
    const resolveWebSocketUrl = () => {
      ...
      const storedPort = readStorageValue(PROMPT_WEBSOCKET_PORT_STORAGE_KEY)?.trim();
      const storedChannel = readStorageValue(PROMPT_WEBSOCKET_CHANNEL_STORAGE_KEY)?.trim();

      if (!storedHost && !storedPort && !storedChannel) {
        return DEFAULT_PROMPT_WEBSOCKET_URL;
      }

      const rawPort = storedPort;
...
`
    ...
    try {
      const historyMessages = conversationWithNewMessage
        .slice(0, -1)
        .map(({ role, displayContent, content }) => ({
          role,
          content: stripCodeBlocks(displayContent || content),
        }))
        .filter((message) => message.content);
      const latestMessage = conversationWithNewMessage[conversationWithNewMessage.length - 1];
